...
ID: user*id, tv_show_id, channel_id.  
...
CF/Seq scores: normalized [0,1] for each score*\* (TopPop/KNN/ALS/BPR/LightGCN/Markov/SASRec/TFIDF/SBERT).
...
## Các Thuật Toán Recommendation trong src/

Dự án sử dụng đa dạng các phương pháp recommendation được tổ chức thành các thư mục chính:

### 1. **Item-based Collaborative Filtering (Item-based CF kNN)**

**Thư mục:** `src/Item-based_CF_kNN.ipynb/`  
**Mô tả:** Phương pháp CF dựa trên sự tương tự giữa các mục (item).

- **Cách hoạt động:** Tính similarity matrix giữa các tv_show dựa trên co-occurrence (người dùng xem chương trình nào).
- **Metric:** Cosine similarity + Shrinkage (để xử lý sparsity).
- **Hyperparameter:** K=200 (số lân cận), λ=50–200 (shrinkage parameter).
- **Ưu điểm:** Đơn giản, nhanh, không cần training.
- **Nhược điểm:** Không capture được preference của user, yêu cầu sufficient co-occurrence.

### 2. **Weighted Matrix Factorization (WMF) với ALS**

**Thư mục:** `src/WMF/WMF_with_ALS.ipynb`  
**Mô tả:** Phân tích nhân tử (matrix factorization) với implicit feedback.

- **Cách hoạt động:** Phân tích ma trận user×item thành hai ma trận nhân tử. Dùng ALS (Alternating Least Squares) để tối ưu.
- **Implicit feedback:** Confidence từ duration xem (càng xem lâu → độ tin cậy cao).
- **Hyperparameter:**
  - factors = 64–128 (chiều embedding)
  - reg = 0.05 (regularization)
  - iters = 15 (số lần lặp ALS)
  - α = 0.01–0.1 (confidence weighting parameter)
- **Ưu điểm:** Hiệu quả với large-scale implicit data, capture user & item factors.
- **Nhược điểm:** Không sử dụng được content features, khó interpret.

### 3. **Bayesian Personalized Ranking (BPR) & LightFM**

**Thư mục:** `src/BPR_LightFM/BPR_LightFM.ipynb`  
**Mô tả:** Pairwise ranking với khả năng kết hợp side features (metadata).

- **Cách hoạt động:**
  - BPR: Optimize pairwise ranking (positive item > negative item).
  - LightFM: Mở rộng BPR với embeddings cho content features (genres, actors, director).
- **Hyperparameter:**
  - factors = 128 (embedding dimension)
  - learning_rate = 1e-3
  - regularization = 1e-4
  - neg_samples = 5 (negative samples per positive)
- **Ưu điểm:** Hoạt động tốt trên implicit data, có thể tích hợp side information.
- **Nhược điểm:** Yêu cầu negative sampling, training time lâu.

### 4. **NeuMF (Neural Collaborative Filtering)**

**Thư mục:** `src/NeuMF/NeuMF.ipynb`  
**Mô tả:** Kết hợp GMF (Generalized Matrix Factorization) + MLP (Multi-Layer Perceptron).

- **Cách hoạt động:**
  - GMF: Linear interaction (element-wise product) của user & item embeddings.
  - MLP: Deep learning layers để capture non-linear interactions.
  - Fusion: Concatenate GMF + MLP outputs, sigmoid để score.
- **Kiến trúc:**
  - User/Item embedding layers
  - GMF: element-wise multiply
  - MLP: Dense layers [256, 128, 64], ReLU activation, Dropout=0.2
  - Output: 1 neuron, sigmoid
- **Training:** Negative sampling, Adam optimizer, learning_rate ≈ 1e-3.
- **Ưu điểm:** Capture cả linear & non-linear interactions, flexible.
- **Nhược điểm:** Tốn tài nguyên training, cần careful hyperparameter tuning.

### 5. **Content-based Recommendation**

#### a. **Content Weighted**

**Thư mục:** `src/content_weighted/cont_weighted.ipynb`  
**Mô tả:** Weighted combination của content features.

- **Features:** TF-IDF/BM25 từ title, actors, director, genres, category.
- **Cách hoạt động:** Tính weighted score dựa trên overlap content features với user history.
- **Hyperparameter:** Tunable weights cho mỗi content type.
- **Ưu điểm:** Cold-start handling, explainable.

#### b. **Content + Knowledge Graph Attention**

**Thư mục:** `src/content_knowledge_graph_attention/cont_kgat.ipynb`  
**Mô tả:** Kết hợp content features + knowledge graph embeddings + attention mechanism.

- **Cách hoạt động:**
  - Xây dựng Knowledge Graph: program–genre–actor–director–channel (multi-hop relations).
  - GCN/GAT encoder: Tạo item embeddings từ KG structure + attributes.
  - Attention: Weight các factors (title similarity, genre overlap, actor overlap, etc.).
- **Hyperparameter:**
  - KG embedding dimension: 64–128
  - GCN/GAT layers: 2–3
  - Attention heads: 2–4
- **Ưu điểm:** Rich contextual signals, capture complex relationships.
- **Nhược điểm:** Phức tạp, yêu cầu KG construction, interpretability khó.

### 6. **Hybrid Recommendation**

**Thư mục:** `src/hybrid/hybrid-4 (3).ipynb`  
**Mô tả:** Kết hợp nhiều phương pháp (CF + content + session-based + deep learning).

- **Cách hoạt động:** Ensemble/pooling scores từ các candidate sources khác nhau.
- **Sources:**
  - CF: ALS, BPR, WMF scores.
  - Content: TF-IDF, SBERT embedding cosine similarity.
  - Session-based: Markov, SASRec predictions.
  - Deep learning: Two-Tower, NeuMF scores.
- **Pooling strategy:** Top-100 từ mỗi source → merge & dedup → cắt ≤ 300–500 candidates/user.
- **Ưu điểm:** Diversity, capture nhiều signals, robust.
- **Nhược điểm:** Phức tạp, yêu cầu compute cao.
